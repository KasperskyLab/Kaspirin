...
<xsl:transform xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
	...
	<xsl:template match="Bullets/Bullet">
		...
		<xsl:if test="$shouldProcess = 'true'">
			<xsl:element name="CustomizationDictionary">
				...
				<ResourceDictionary xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
					...
					<!--BulletColor-->
					<xsl:if test="BulletColor">
						<xsl:call-template name="generateBrush">
							<xsl:with-param name="key">BulletColor</xsl:with-param>
							<xsl:with-param name="brushNode" select="BulletColor"/>
						</xsl:call-template>
					</xsl:if>

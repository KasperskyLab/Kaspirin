...
<xsl:transform xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
	...
	<xsl:template match="Bullets">
		...
		<xsl:if test="$shouldProcess = 'true'">
			<xsl:call-template name="getRegionComment">
				<xsl:with-param name="description">Bullets</xsl:with-param>
			</xsl:call-template>
	...
	<xsl:template match="Bullet">
		...
		<xsl:call-template name="generateStyle">
			<xsl:with-param name="targetType">visuals:Bullet</xsl:with-param>
			<xsl:with-param name="basedOn">BulletUniversal</xsl:with-param>
			...
			<xsl:with-param name="setters">
				...
				<xsl:call-template name="generateBrushSetter">
					<xsl:with-param name="propertyId">Bullet_Color</xsl:with-param>
					<xsl:with-param name="brushName">BulletColor</xsl:with-param>
					<xsl:with-param name="scopeName" select="$id"/>
				</xsl:call-template>

				<!--Bullet_Margin-->
				<xsl:call-template name="generateMarginSetter">
					<xsl:with-param name="propertyId">Bullet_Margin</xsl:with-param>
					<xsl:with-param name="marginNode" select="BulletMargin" />
				</xsl:call-template>

				<!--Bullet_TextStyle-->
				<xsl:call-template name="generateStaticResourceSetter">
					<xsl:with-param name="propertyId">Bullet_TextStyle</xsl:with-param>

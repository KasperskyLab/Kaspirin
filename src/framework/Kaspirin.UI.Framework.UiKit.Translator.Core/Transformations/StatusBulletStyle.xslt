...
<xsl:transform xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
	...
	<xsl:template match="StatusBullets">
		...
		<xsl:if test="$shouldProcess = 'true'">
			<xsl:call-template name="getRegionComment">
				<xsl:with-param name="description">StatusBullets</xsl:with-param>
			</xsl:call-template>
	...
	<xsl:template match="StatusBullet">
		...
		<xsl:call-template name="generateStyle">
			<xsl:with-param name="targetType">visuals:StatusBullet</xsl:with-param>
			<xsl:with-param name="basedOn">StatusBulletUniversal</xsl:with-param>
			...
			<xsl:with-param name="setters">
				...
				<xsl:call-template name="generateMarginSetter">
					<xsl:with-param name="propertyId">StatusBullet_Margin</xsl:with-param>
					<xsl:with-param name="marginNode" select="BulletMargin" />
				</xsl:call-template>

				<!--StatusBullet_Mark-->
				<xsl:call-template name="generateUiKitSetterViaAttribute">
					<xsl:with-param name="propertyId">StatusBullet_Mark</xsl:with-param>
					<xsl:with-param name="propertyValue">
						<xsl:call-template name="generateImgExtension">
							<xsl:with-param name="key">
								<xsl:call-template name="getSvgFilename">
									<xsl:with-param name="id" select="$id" />
									<xsl:with-param name="member" select="name(BulletMark)" />
								</xsl:call-template>
							</xsl:with-param>
						</xsl:call-template>
					</xsl:with-param>
				...
				<!--StatusBullet_TextStyle-->
				<xsl:if test="@TextStyleId">
					<xsl:call-template name="generateUiKitSetterViaAttribute">
						<xsl:with-param name="propertyId">StatusBullet_TextStyle</xsl:with-param>
						<xsl:with-param name="propertyValue">
				...
				<xsl:call-template name="generateTrigger">
					...
					<xsl:with-param name="setters">
						...
						<xsl:if test="BulletColorDanger">
							<xsl:call-template name="generateUiKitSetterViaAttribute">
								<xsl:with-param name="propertyId">StatusBullet_Color</xsl:with-param>
								<xsl:with-param name="propertyValue">
									<xsl:call-template name="generateResExtension">

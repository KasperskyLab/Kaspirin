...
<xsl:transform xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
	...
	<xsl:template match="TextInputs">
		...
		<xsl:if test="$shouldProcess = 'true'">
			<xsl:call-template name="getRegionComment">
				<xsl:with-param name="description">TextInputs</xsl:with-param>
			</xsl:call-template>
	...
	<xsl:template match="TextInput">
		...
		<xsl:element name="Style">
			<xsl:attribute name="TargetType">{x:Type visuals:TextInput}</xsl:attribute>
			...
			<!--TextInput_ActionBar_Margin-->
			<xsl:call-template name="generateMarginSetter">
				<xsl:with-param name="propertyId">TextInput_ActionBar_Margin</xsl:with-param>

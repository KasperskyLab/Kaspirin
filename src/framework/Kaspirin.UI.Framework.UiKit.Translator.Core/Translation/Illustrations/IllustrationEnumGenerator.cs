...
internal sealed class IllustrationEnumGenerator
{
    ...
    private IDictionary<string, Illustration[]> GetIllustrations(string uiKitContent) =>
        ...
        .GroupBy(i => i.Product)
        .ToDictionary(g => g.Key, g => g.Distinct(new IllustrationEqualityComparer()).ToArray());
    ...
    private string FillEnums(Illustration[] illustrations, string enumsFile)
    {
        ...
        foreach (var g in illustrations.GroupBy(i => i.Scope))
        {
            ...
            illustrationEnumsStringBuilder.AppendLine($"/// </summary>UIKit illustrations with {scope} scope.</summary>");
            illustrationEnumsStringBuilder.AppendLine($"public enum {enumTypeName}");
            illustrationEnumsStringBuilder.AppendLine($"{{");

            illustrationEnumsStringBuilder.AppendLine($"{_indent}UIKitUnset = 0,");

            foreach (var illustration in g)
            {
                illustrationEnumsStringBuilder.AppendLine($"{_indent}{illustration.Name},");
            }

            illustrationEnumsStringBuilder.Append($"}}");
        }
    ...
    private sealed class IllustrationEqualityComparer : IEqualityComparer<Illustration>
    {
        public bool Equals(Illustration x, Illustration y)
        {
            return x.Name == y.Name && x.Scope == y.Scope && x.Product == y.Product;
        }

        public int GetHashCode(Illustration obj)
        {
            return obj.Name.GetHashCode() ^ obj.Scope.GetHashCode() ^ obj.Product.GetHashCode();
        }
    }

...
internal sealed class IllustrationSvgGenerator : GeneratorBase
{
    ...
    private IEnumerable<string> GetSvgPaths(string outputDirectory, string scope, string name, string prefix, bool isRTL, string theme)
    {
        ...
        foreach (var locale in locales)
        {
            ...
            var prefixSubdirectory = string.IsNullOrWhiteSpace(prefix)
                ? string.Empty
                : $@"{prefix}\";

            var directory = Path.Combine(outputDirectory, Path.Combine(locale, $@"{prefixSubdirectory}{themeSubdirectory}images\{scope}").ToLower());

...
public sealed class WeakEventHandlerFactory<TEventHandler> where TEventHandler : Delegate
{
    ...
    ///     <br /> The weak link is checked every time the delegate is called <paramref name="EventHandler" />
    ///     and if the weak link is deleted, then the delegate <paramref name="cleanup" /> is called in this case.
    /// </remarks>
    /// <returns>
    ...
    private WeakEventHandlerFactory(Delegate d, Action<TEventHandler> cleanup)
    {
        ...
        var openEventHandlerGenericType = parameterTypes.Length switch
        {
            ...
            6 => typeof(Action<,,,,,,>),
            7 => typeof(Action<,,,,,,,>),
            _ => throw new UnexpectedValueException(parameterTypes.Length)
        };

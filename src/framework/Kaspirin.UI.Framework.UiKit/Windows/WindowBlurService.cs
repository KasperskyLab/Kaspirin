...
public sealed class WindowBlurService
{
    ...
    private void DisableBlur()
    {
        if (IsBlurSupported)
        {
            SetWindowAccentState(AccentState.Disabled);
        }
    ...
    private bool SetWindowAccentState(AccentState accentState)
    {
        ...
        var accentPolicy = new AccentPolicy
        {
            ...
            GradientColor = ToAbgrHex(_defaultGradientColor)
        };
        ...
        try
        {
            ...
            {
                _tracer.TraceError($"Unable to set {accentState} accent state for window {_window.GetType().Name}." +
        ...
        {
            _tracer.TraceError($"Unable to set {accentState} accent state for window {_window.GetType().Name}." + Environment.NewLine + ex);
            return false;
        }
        finally
        {
    ...
    {
        if (_currentAccentState == AccentState.EnableBlurBehind && BlurBackground != null)
        {
            Guard.EnsureIsNotNull(_window).Background = BlurBackground;
        }
        else if (_currentAccentState == AccentState.EnableAcrylicBlurBehind && AcrylicBlurBackground != null)
        {
    ...
    private static readonly Color _defaultGradientColor = Colors.Transparent;

    private static readonly ComponentTracer _tracer = ComponentTracer.Get(nameof(WindowBlurService));
}
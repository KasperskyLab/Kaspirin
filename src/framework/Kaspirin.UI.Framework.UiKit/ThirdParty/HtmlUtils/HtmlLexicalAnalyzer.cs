...
namespace Kaspirin.UI.Framework.UiKit.ThirdParty.HtmlUtils
{
    ...
    internal sealed class HtmlLexicalAnalyzer
    {
        ...
        private void GetNextCharacter()
        {
            ...
            if (NextCharacter == '&')
            {
                if (_lookAheadCharacter == '#')
                {
                    ...
                    if (_lookAheadCharacter == 'x')
                    {
                        var hexString = "0x";
                        ReadLookAheadCharacter();
                        // largest hex numeric entity is 4 characters
                        for (var i = 0; i < 4 && IsHexDigit(_lookAheadCharacter); i++)
                        {
                            hexString += _lookAheadCharacter.ToString();
                            ReadLookAheadCharacter();
                        }

                        entityCode = Convert.ToInt32(hexString, 16);
                    }
                    else
                    {
        ...
        private bool IsGoodForName(char character) => IsGoodForNameStart(character) ||
                ...
                // A part of conditional expression is always false if it is evaluated
                IsCombiningCharacter(character) || //-V3063
                IsExtender(character); //-V3063
        ...
        private void ReadDynamicContent()
        {
            ...
            Guard.Assert(_previousCharacter == '<' && NextCharacter == '!' && _lookAheadCharacter == '[');
        ...
        private void ReadUnknownDirective()
        {
            ...
            Guard.Assert(_previousCharacter == '<' && NextCharacter == '!' &&
        ...
        private void SkipProcessingDirective()
        {
            ...
            Guard.Assert(NextCharacter == '<' && _lookAheadCharacter == '?');

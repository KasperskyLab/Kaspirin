...
internal sealed class SelectItemInfoProvider : DependencyObject
{
    ...
    public SelectItem GetContainer()
    {
        ...
        if (TryGetContainer(out container))
        {
            return container;
        }
    ...
    private bool TryGetContainer([NotNullWhen(true)] out SelectItem? container)
    {
        if (_container.TryGetTarget(out var target) && IsValidContainer(target))
        {
            container = target;
            return true;
        }
    ...
    private static bool IsValidContainer(SelectItem container)
    {
        var dataContextTypeName = container.DataContext?.GetType().FullName;
        var dataContextText = container.DataContext?.ToString();

        return !string.Equals(dataContextTypeName, "MS.Internal.NamedObject") &&
               !string.Equals(dataContextText, "{DisconnectedItem}");
    }
    ...
    private static readonly DependencyProperty _isFakeContainerProperty = DependencyProperty.RegisterAttached(
        "IsFakeContainer",
        typeof(bool),
        typeof(SelectItemInfoProvider));
}
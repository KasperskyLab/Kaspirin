...
public sealed class PasswordInput : Control, ISecureInputClient
{
    ...
    public static readonly DependencyProperty ActionBarProperty = DependencyProperty.Register(
        ...
        new PropertyMetadata(default(InputActionCollection)));
    ...
    public static readonly DependencyProperty IsPasswordRevealedProperty = DependencyProperty.Register(
        ...
        typeof(bool),
        typeof(PasswordInput),
        new FrameworkPropertyMetadata(default(bool), FrameworkPropertyMetadataOptions.BindsTwoWayByDefault, OnIsPasswordRevealedChanged));
    ...
    public bool HandleKeyPress(SecureInputAction inputAction)
    {
        ...
        var isControlKeysNotPressedExceptShift = _pressedControlKeys.All(k => k == VirtualKey.Shift);
    ...
    {
        if (_inputPasswordElement is null)
        {
            return;
        }

        var password = _inputPasswordElement.SecurePassword;
        if (Password.IsEqualTo(password))
        {
            return;
        }
    ...
    private void TryReadPasswordFromOpenPasswordTextBox()
    {
        ...
        var password = _openPasswordTextBox.Text?.ToSecureString();
        if (Password.IsEqualTo(password))
        {
            return;
        }

...
using TResult = System.Collections.Generic.KeyValuePair<string, Kaspirin.UI.Framework.UiKit.Localization.Localizer.Strings.Parsing.Expressions.ValueExpression>;

namespace Kaspirin.UI.Framework.UiKit.Localization.Localizer.Strings.Parsing.Evaluators;

internal sealed class ValueExpressionEvaluator : IKeyValueEvaluator<TResult, IExpression, VariableExpression, IExpression, IExpression>
{
    public ValueExpressionEvaluator(CultureInfo cultureInfo) => _cultureInfo = cultureInfo;

    public TResult KeyValue(Token key, Token equals, IExpression expression, Token? comment, Token? newLine) => new(key.GetText(), (ValueExpression)expression);
    ...
    public IExpression PluralForm(
        ...
        new PluralExpression(
            selector: param,
            pluralForms: pluralFormExpressions.Take(PluralFormInfoProvider.GetFormCount(_cultureInfo)).Cast<ValueExpression>().ToArray(),
            cultureInfo: _cultureInfo);

    public IExpression PluralFormExpression(IEnumerable<IExpression> operands) => new ValueExpression(operands.ToArray());

    public IExpression Expression(IEnumerable<IExpression> operands) => new ValueExpression(operands.ToArray());

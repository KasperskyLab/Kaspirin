...
{
    public Position(int start, int end, string sourceName, string sourceText)
    {
        Guard.Argument(start >= 0);
        Guard.Argument(end >= 0);
        Guard.Argument(start <= end);
    ...
    {
        Guard.Assert(a.SourceName == b.SourceName);
        Guard.Assert(object.ReferenceEquals(a.SourceText, b.SourceText));

        return new Position(Math.Min(a.Start, b.Start), Math.Max(a.End, b.End), a.SourceName, a.SourceText);
    }
    ...
    public string ToString(string sourceText)
    {
        ...
        return $"{StartLineColumn.Line}:{StartLineColumn.Column} ({Length}): \"{text}\" SourceName: '{SourceName}'";
    }
}
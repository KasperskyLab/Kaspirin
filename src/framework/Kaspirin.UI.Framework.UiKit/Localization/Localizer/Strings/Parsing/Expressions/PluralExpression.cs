...
public sealed class PluralExpression : IExpression
{
    ...
    public string Resolve(Func<string, ValueExpression> keyResolver, Func<string, object?> variableResolver)
    {
        ...
        var formNumber = PluralFormInfoProvider.GetFormNumber(_cultureInfo, Convert.ToInt32(result));

        if (formNumber >= PluralForms.Length)
        {
            var formCount = PluralFormInfoProvider.GetFormCount(_cultureInfo);

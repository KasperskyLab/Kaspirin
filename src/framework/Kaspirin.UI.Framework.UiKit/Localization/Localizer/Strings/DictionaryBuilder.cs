...
namespace Kaspirin.UI.Framework.UiKit.Localization.Localizer.Strings
{
    ...
    {
        public static IReadOnlyDictionary<string, ValueExpression> BuildScope(string scopeText, CultureInfo cultureInfo, string filePath)
        {
            try
            {
                return Parser.ParseScope(scopeText, new ValueExpressionEvaluator(cultureInfo), filePath)
                    .ToImmutableDictionary(StringComparer.InvariantCultureIgnoreCase);
            }
            catch (Exception e)
            {
                e.ProcessAsLocalizerException(e.Message);
                return ImmutableDictionary.Create<string, ValueExpression>(StringComparer.OrdinalIgnoreCase);
            }
        }
    }
}

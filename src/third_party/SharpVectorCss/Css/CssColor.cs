    ...
    public sealed class CssColor : ICssColor
    {
        ...
        private void ParseColor(string str)
        {
            ...
            else
            {
                ...
                if (_knownColors.ContainsKey(str))
                {
                    ...
                    SetPrimitiveValues(int.Parse(color.Substring(0, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo),
                        int.Parse(color.Substring(2, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo),
                        int.Parse(color.Substring(4, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo));
                }
        ...
        private struct ColorRGBA
        {
            ...
            public static ColorRGBA FromHex(string value)
            {
                ...
                switch (color.Length)
                {
                    case 3:
                        ...
                        r = int.Parse(rgbColor.Substring(0, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        g = int.Parse(rgbColor.Substring(2, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        b = int.Parse(rgbColor.Substring(4, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        break;
                    case 4:
                        var argbColor = string.Format("{0}{0}{1}{1}{2}{2}{3}{3}", color[0], color[1], color[2], color[3]);
                        r = int.Parse(argbColor.Substring(0, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        g = int.Parse(argbColor.Substring(2, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        b = int.Parse(argbColor.Substring(4, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        a = int.Parse(argbColor.Substring(6, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        break;
                    case 6:
                        r = int.Parse(color.Substring(0, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        g = int.Parse(color.Substring(2, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        b = int.Parse(color.Substring(4, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        break;
                    case 8:
                        r = int.Parse(color.Substring(0, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        g = int.Parse(color.Substring(2, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        b = int.Parse(color.Substring(4, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        a = int.Parse(color.Substring(6, 2), NumberStyles.HexNumber, NumberFormatInfo.InvariantInfo);
                        break;
                }
            ...
            public static ColorRGBA FromHsl(string hue, string saturation, string lightness, string alpha = null)
            {
                ...
                {
                    h = double.Parse(hue, NumberFormatInfo.InvariantInfo) / 360.0;
                }
                else if (string.Equals(hueUnit, "rad", StringComparison.Ordinal))  // Radian
                {
                    h = Math.Round(double.Parse(hue, NumberFormatInfo.InvariantInfo) * 180 / Math.PI, 2) / 360.0;
                }
                ...
                {
                    h = Math.Round(double.Parse(hue, NumberFormatInfo.InvariantInfo) * 180 / 200, 2) / 360.0;
                }
                else if (string.Equals(hueUnit, "turn", StringComparison.Ordinal))  // Turns
                {
                    h = double.Parse(hue, NumberFormatInfo.InvariantInfo);
                }

                double s = double.Parse(saturation, NumberFormatInfo.InvariantInfo) / 100.0;
                double l = double.Parse(lightness, NumberFormatInfo.InvariantInfo) / 100.0;
                ...
                if (!string.IsNullOrWhiteSpace(alpha))
                {
                    ...
                    {
                        alphaValue = double.Parse(alpha.TrimEnd('%'), NumberFormatInfo.InvariantInfo) / 100.0;
                    }
                    else
                    {
                        alphaValue = double.Parse(alpha, NumberFormatInfo.InvariantInfo);
                    }
                    if (NumOperations.GreaterOrEquals(alphaValue, 0) && NumOperations.LesserOrEquals(alphaValue, 1))
                    {
            ...
            public static ColorRGBA FromHsl(double h, double s, double l, int a = -1)
            {
                ...
                double g = l;
                double b = l;
                double v = NumOperations.LesserOrEquals(l, 0.5) ? (l * (1.0 + s)) : (l + s - l * s);
                if (v > 0)
                {
                    double m;
                    double sv;

...
namespace SharpVectors.Renderers.Wpf
{
    public class WpfRendering : WpfRenderingBase
    {
                ...
                hint == SvgRenderingHint.Containment || hint == SvgRenderingHint.Image)
            {
                ...
                if (clipPath != null && clipPath.PrimitiveType == CssPrimitiveType.Uri)
                {
                    ...
                    if (eClipPath != null)
                    {
                        ...
                        if (geomColl == null || geomColl.Count == 0) //-V3063
                        {
                            return;
                        }
        ...
        protected void SetMask(WpfDrawingContext context)
        {
            ...
            {
                var parentElement = (SvgElement)_svgElement.ParentNode;
        ...
        private static Rect GetMaskDestRect(SvgMaskElement maskElement, Rect bounds)
        {
            ...
            result.X = CalcPatternUnit(maskElement, (SvgLength)maskElement.X.AnimVal,
                SvgLengthDirection.Horizontal, bounds);
            result.Y = CalcPatternUnit(maskElement, (SvgLength)maskElement.Y.AnimVal,
                SvgLengthDirection.Vertical, bounds);

            result.Width = CalcPatternUnit(maskElement, (SvgLength)maskElement.Width.AnimVal,
                SvgLengthDirection.Horizontal, bounds);
            result.Height = CalcPatternUnit(maskElement, (SvgLength)maskElement.Height.AnimVal,
                SvgLengthDirection.Vertical, bounds);
            ...
            SvgStyleableElement styleElm, WpfDrawingContext gr)
        {
            ...
            if (styleElm is ISharpMarkerHost)
            {
                ...
                {
                    using (var markerRenderer = CreateByUri(styleElm.OwnerDocument,

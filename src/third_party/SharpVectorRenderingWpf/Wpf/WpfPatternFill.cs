    ...
    public sealed class WpfPatternFill : WpfFill
    {
        ...
        public override Brush GetBrush(Rect elementBounds, WpfDrawingContext context, Transform viewTransform)
        {
            ...
            if (isUserSpace)
            {
                ...
                if (transform != null && !transform.Matrix.IsIdentity) //-V3063
                {
        ...
        private void PrepareTargetPattern()
        {
            ...
            {
                SvgPatternElement svgElm = (SvgPatternElement)_patternElement.CloneNode(true);
        ...
        private DrawingGroup GetImage(WpfDrawingContext context)
        {
            ...
            using (WpfDrawingRenderer renderer = new WpfDrawingRenderer())
            {
                ...
                if (SvgUnitType.ObjectBoundingBox.Equals(_renderedElement?.PatternContentUnits?.AnimVal))
                {
                    //                svgElm.SetAttribute("viewBox", "0 0 1 1");
                }
                else
        ...
        private Rect GetDestRect(Rect bounds)
        {
            ...
            result.X = CalcPatternUnit((SvgLength)_patternElement.X.AnimVal,
                SvgLengthDirection.Horizontal, bounds);
            result.Y = CalcPatternUnit((SvgLength)_patternElement.Y.AnimVal,
                SvgLengthDirection.Vertical, bounds);

            result.Width = CalcPatternUnit((SvgLength)_patternElement.Width.AnimVal,
                SvgLengthDirection.Horizontal, bounds);
            result.Height = CalcPatternUnit((SvgLength)_patternElement.Height.AnimVal,
                SvgLengthDirection.Vertical, bounds);
